...
### 1.1 页面骨架与导航 (P0)
- [ ] **TASK-101** [P0] 实现悬浮式磨砂底栏与三 Tab 页面骨架 (`sdd/01-app-home-and-navigation`)
  - [ ] 首页 (Home)、对话 (Chat)、我的 (Me)
  - [ ] 首页英雄卡（孕16周插画与孕期阶段 Chip）、四角色 2×2 网格（小暖激活，其余锁定展示）
- [ ] **TASK-102** [P0] 实现「我的」与隐私设置弹窗 (`sdd/16-privacy-compliance`)
  - [ ] 设置列表项（隐私说明、数据导出、删除数据、编辑档案）
  - [ ] 磨砂玻璃 Sheet 弹窗交互与用户授权状态持久化
...
### 2.1 本地知识库与记忆切片 (P1)
- [ ] **TASK-201** [P1] 本地知识检索与 RAG 插件 (`sdd/07-local-knowledge-retrieval`)
  - [ ] 内置孕期常见食物红黑榜、孕检时间表文档
  - [ ] 本地关键词 / 向量检索节点，回答时附带「来源: 权威指南」标牌
- [ ] **TASK-202** [P1] 上下文记忆切片服务 (`sdd/09-context-slice-memory`)
  - [ ] 维护用户近 7 天情绪、孕周、睡眠与身体习惯摘要
  - [ ] 动态拼接至 Prompt 上下文，避免超长 Token 浪费，保证陪伴连续性

### 2.2 四角色解锁与接力会诊 (P1)
- [ ] **TASK-203** [P1] 激活林医生、苏心、阿嬷三位专家 Agent (`sdd/11-four-roles-and-handoff`)
  - [ ] 林医生（医学产检、症状分析、工具调用）
  - [ ] 苏心（产前焦虑、产后情绪、身心放松引导）
  - [ ] 阿嬷（生活偏方纠偏、坐月子饮食、习俗关怀）
- [ ] **TASK-204** [P1] 多角色群聊会诊机制 (Handoff) (`sdd/11-four-roles-and-handoff`)
  - [ ] 主持人路由机制：小暖判断专业问题自动 @林医生 或 @苏心
  - [ ] 消息列表支持多角色归属展示（带对应角色头像与专属气泡标识）
