...
> **Project override (ai_mom_baby):** State/DI = Riverpod. HTTP = Dio. Local DB = Drift. Ignore examples that use `provider`, `ChangeNotifier`+`package:provider`, `package:http`, or `sqflite`. Map teaching ViewModels to Riverpod `Notifier` / `AsyncNotifier` (or existing project patterns).
